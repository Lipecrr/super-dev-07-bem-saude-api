...
Define os DTOs (Data Trasnfer Objects) para requisições e respostas relacionadas
a recepcionistas.
"""
...
class RecepcionistaResponse(BaseModel):
    """"
    Schema de resposta recepcionista
    ...
    nome: str = Field(
        ...,
        description="Nome completo do recpcionista",
        examples=["Mario dos Santos"]
    )
    status: StatusCadastro = Field(
        ..., # Campo obrigatório e n~~ao tem valor padrão
        description="Status do cadastro",
        examples=["ATIVO"]
    )
